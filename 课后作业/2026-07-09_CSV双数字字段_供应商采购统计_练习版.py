...
for line in lines:
    ...
    if clean_line != "" and clean_line != "供应商名称,采购数量,采购金额":
        raw_record_list.append(clean_line)


# 第 5 块：准备结果列表和统计变量
valid_supplier_name_list = []
valid_purchase_qty_list = []
valid_purchase_amount_list = []

high_purchase_list = []
normal_purchase_list = []
zero_purchase_list = []
...
for record in raw_record_list:
    ...
    if len(parts) != 3:
        ...
        purchase_qty_text = parts[1].strip()
...
# 判断和转换
        ...
        if supplier_name == "":
        ...
        elif not purchase_qty_text.isdigit() :
            invalid_count += 1

        elif not purchase_amount_text.replace(".", "", 1).isdigit():
            invalid_count += 1

        else:
            purchase_qty = int(purchase_qty_text)

            purchase_amount = float(purchase_amount_text)

            valid_supplier_name_list.append(supplier_name)

            valid_purchase_qty_list.append(purchase_qty)
            ...
            if purchase_amount >= 20000:
                high_purchase_list.append(supplier_name)
            elif purchase_amount > 0:
                normal_purchase_list.append(supplier_name)
            else:
                zero_purchase_list.append(supplier_name)


# 第 7 块：统计数量和平均值
raw_count = len(raw_record_list)

valid_count = len(valid_supplier_name_list)

if valid_count > 0:
    avg_purchase_qty = total_purchase_qty / valid_count
...
with open(cleaned_file, "w", encoding="gbk") as f:
    f.write("供应商名称,采购数量,采购金额\n")

    for i in range(valid_count):
        f.write(
            valid_supplier_name_list[i]
            + ","
            + str(valid_purchase_qty_list[i])
            + ","
...
with open(summary_file, "w", encoding="utf-8") as f:
    f.write("CSV 供应商采购清洗报告\n")
    f.write("====================\n")
    f.write("原始记录数量：" + str(raw_count) + "\n")
    f.write("有效记录数量：" + str(valid_count) + "\n")
    f.write("无效记录数量：" + str(invalid_count) + "\n")
    f.write("总采购数量：" + str(total_purchase_qty) + "\n")
    f.write("总采购金额: " + str(total_purchase_amount) + "\n")
    f.write("平均采购数量：" + str(round(avg_purchase_qty, 2)) + "\n")
    f.write("平均采购金额：" + str(round(avg_purchase_amount, 2)) + "\n")
    f.write("大额采购数量: " + str(len(high_purchase_list)) + "\n")
    f.write("普通采购数量: " + str(len(normal_purchase_list)) + "\n")
    f.write("零采购数量: " + str(len(zero_purchase_list)) + "\n")