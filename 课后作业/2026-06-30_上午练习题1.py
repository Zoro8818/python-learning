...
score_text = " 小王:90, 小李:60, , 小赵:30, 小周:100, 小吴:0, 错误数据, 小陈:abc, :80, 小刘: "
...
for record in score_list:
    ...
    if clean_record == "":
        ...
        score_text_item = parts[1].strip()

        if student == "":
            invalid_count += 1
        elif score_text_item == "":
            invalid_count += 1
        elif not score_text_item.replace(".", "", 1).isdigit():
            invalid_count += 1
        else:
            score = float(score_text_item)
...
print("高分学生数量: ", high_score_count)
print("及格学生列表: ", pass_student_list)
print("及格学生数量: ", pass_count)
print("不及格学生列表: ", fail_student_list)
print("不及格学生数量: ", fail_count)