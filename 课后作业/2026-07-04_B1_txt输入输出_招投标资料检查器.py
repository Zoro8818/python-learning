# 2026-07-04 B1：txt 输入输出版招投标资料检查器
# 学习定位：从手写 raw_submitted_list 推进到真实 txt 输入 + txt 报告输出。
# 输入文件：input/input.txt
# 输出文件：output/report.txt
# 说明：本练习脚本可在本机使用绝对路径；通用模板仍使用相对路径。
...
with open(report_file, "w", encoding="utf-8") as f:
    ...
    f.write("客户已提交的必备资料包括：" + str(checked_submitted_list) + "\n")
    f.write("目前仍缺失资料：" + str(missing_list) + "\n")
    f.write("缺失关键必备资料包括：" + str(key_missing_list) + "\n")
    f.write("另发现多余资料：" + str(extra_list) + "\n")
    f.write("无效资料包括：" + str(invalid_list) + "\n")
    f.write("重复提交资料包括：" + str(duplicate_list) + "\n")
